...
## Watch history / recommendations tracking

The background WebView player now genuinely plays alongside ExoPlayer,
muted and invisible, instead of being paused. This is what makes YouTube's
own watch-history, view count, and recommendation algorithm register videos
as watched when the user is logged into YouTube in the WebView — it's real
playback through the user's actual session, not fabricated telemetry.
`googlevideo.com` is no longer blocked on watch pages for this reason (see
`AdBlockEngine`'s comments). `injected.js`'s `disableNativeAutoplay()`
prevents the background player from independently navigating to a
different video than ExoPlayer if it happens to buffer faster — all
video-to-video navigation goes through the native side's
`advanceToNextVideo()` only, keeping both players pointed at the same video.

Explicitly NOT implemented: any form of fabricating/spoofing watch-time or
view signals directly to YouTube's servers. That's a different thing from
"let the real player really play" and wasn't something this project takes on.
