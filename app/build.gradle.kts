...
dependencies {
    ...
    // InMobi — powers the optional "Support the Developer" rewarded-ad
    // button. Chosen over AdMob because InMobi pays out via direct bank
    // transfer/EFT to Indian accounts (no PayPal needed) with a $50
    // threshold. Never auto-shown, never on a timer — only fires when the
    ...
    // yt-dlp wrapper -- last-resort fallback for age-restricted videos
    // when both NewPipeExtractor and Piped instances fail (see
    // extractor/YtDlpResolver.kt). Off by default via a Settings toggle
    // since it bundles a Python runtime + yt-dlp (~30-40MB) into the APK.
