package dev.sparkynox.sparkytube.support

import android.app.Activity
import android.os.Handler
import android.os.Looper
import android.util.Log
import com.inmobi.ads.AdMetaInfo
import com.inmobi.ads.InMobiAdRequestStatus
import com.inmobi.ads.InMobiInterstitial
import com.inmobi.ads.listeners.InterstitialAdEventListener
import com.inmobi.sdk.InMobiSdk
import com.inmobi.sdk.SdkInitializationListener
import org.json.JSONObject

object SupportAdManager {

    private const val TAG = "SupportAdManager"

    private const val ACCOUNT_ID = "595db65f209a417c84324df39612b6bd"
    private const val PLACEMENT_ID = 10000756318L

    // If InMobi's own callbacks never fire (account setup incomplete, no
    // network, a hung request, etc.), these timeouts make sure the "Support
    // the Developer" flow always resolves to a visible outcome instead of
    // leaving the caller's loading state stuck forever with no feedback —
    // which is the exact "shows loading but never loads" symptom reported.
    private const val INIT_TIMEOUT_MS = 8_000L
    private const val LOAD_TIMEOUT_MS = 10_000L

    private var initialized = false
    private var interstitial: InMobiInterstitial? = null
    private val timeoutHandler = Handler(Looper.getMainLooper())

    interface ResultListener {
        fun onAdShown()
        fun onAdCompleted()
        fun onAdUnavailable(reason: String)
    }

    fun init(activity: Activity, onReady: () -> Unit = {}, onFailed: (String) -> Unit = {}) {
        if (initialized) {
            onReady()
            return
        }
        try {
            var settled = false
            val timeoutRunnable = Runnable {
                if (!settled) {
                    settled = true
                    Log.w(TAG, "InMobi init timed out after ${INIT_TIMEOUT_MS}ms")
                    onFailed("Support ads aren't responding right now — check back later.")
                }
            }
            timeoutHandler.postDelayed(timeoutRunnable, INIT_TIMEOUT_MS)

            InMobiSdk.init(activity, ACCOUNT_ID, JSONObject(), object : SdkInitializationListener {
                override fun onInitializationComplete(error: Error?) {
                    if (settled) return // timeout already fired, ignore late callback
                    settled = true
                    timeoutHandler.removeCallbacks(timeoutRunnable)

                    if (error == null) {
                        initialized = true
                        onReady()
                    } else {
                        Log.w(TAG, "InMobi init failed: ${error.message}")
                        onFailed("Support ads aren't available right now — check back later.")
                    }
                }
            })
        } catch (e: Exception) {
            Log.w(TAG, "InMobi init threw, support ads unavailable this session", e)
            onFailed("Support ads aren't available right now — check back later.")
        }
    }

    fun showSupportAd(activity: Activity, listener: ResultListener) {
        if (!initialized) {
            listener.onAdUnavailable("Support ads aren't ready yet — try again in a moment.")
            return
        }

        var settled = false
        val timeoutRunnable = Runnable {
            if (!settled) {
                settled = true
                Log.w(TAG, "InMobi ad load timed out after ${LOAD_TIMEOUT_MS}ms")
                listener.onAdUnavailable("No ad available right now — try again in a bit.")
            }
        }
        timeoutHandler.postDelayed(timeoutRunnable, LOAD_TIMEOUT_MS)

        val ad = InMobiInterstitial(activity, PLACEMENT_ID, object : InterstitialAdEventListener() {
            override fun onAdLoadSucceeded(ad: InMobiInterstitial, info: AdMetaInfo) {
                if (settled) return
                settled = true
                timeoutHandler.removeCallbacks(timeoutRunnable)

                if (ad.isReady()) {
                    ad.show()
                } else {
                    listener.onAdUnavailable("No ad available right now — try again in a bit.")
                }
            }

            override fun onAdLoadFailed(ad: InMobiInterstitial, status: InMobiAdRequestStatus) {
                if (settled) return
                settled = true
                timeoutHandler.removeCallbacks(timeoutRunnable)
                listener.onAdUnavailable("No ad available right now — try again in a bit.")
            }

            override fun onAdDisplayed(ad: InMobiInterstitial, info: AdMetaInfo) {
                listener.onAdShown()
            }

            override fun onAdDismissed(ad: InMobiInterstitial) {
                listener.onAdCompleted()
                interstitial = null
            }
        })

        interstitial = ad
        ad.load()
    }
}
