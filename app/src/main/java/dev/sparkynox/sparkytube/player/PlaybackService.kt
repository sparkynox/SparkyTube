...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        val loadControl = DefaultLoadControl.Builder()
            .setBufferDurationsMs(
                /* minBufferMs = */ 15_000,
                /* maxBufferMs = */ 50_000,
                // Lowered from 1000/2000 -- these were forcing ExoPlayer to
                // sit on a fully-buffered chunk before it would even start
                // rendering the first frame, which is backwards for "start
                // playing fast": 500ms of buffered 144p video is more than
                // enough to start smoothly, and cutting this in half was a
                // meaningful chunk of the "takes a while to actually show
                // anything" delay reported at low quality.
                /* bufferForPlaybackMs = */ 500,
                /* bufferForPlaybackAfterRebufferMs = */ 1_000
            )
            // Don't wait to build up a big buffer before allowing playback
            // to actually begin -- prioritize starting fast over starting
            // "safe" with a large buffer cushion already banked.
        ...
        // Notification tap target: without this, tapping the media
        // notification/lock-screen controls does nothing (Media3's
        // default notification has no launch intent unless the session
        // is explicitly given one). FLAG_ACTIVITY_NEW_TASK is required
        // since this PendingIntent fires from outside any activity
        // context (the notification, possibly with the app fully
        // backgrounded/task-less at the time).
        val openAppIntent = Intent(this, dev.sparkynox.sparkytube.MainActivity::class.java).apply {
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP
        }
        val piFlags = android.app.PendingIntent.FLAG_UPDATE_CURRENT or android.app.PendingIntent.FLAG_IMMUTABLE
        val sessionActivityPendingIntent = android.app.PendingIntent.getActivity(this, 0, openAppIntent, piFlags)
    ...
    private fun buildMediaSourceFactory(): MediaSource.Factory {
        val httpDataSourceFactory = DefaultHttpDataSource.Factory()
            ...
            // googlevideo.com stream URLs frequently 30x-redirect to a
            // different edge host (sometimes http->https or vice versa
            // depending on region/CDN) -- without this, ExoPlayer refuses
            // to follow those redirects at all and the connection just
            // fails/stalls until DefaultHttpDataSource gives up, which is
            // part of what was showing up as "stuck for a few seconds
