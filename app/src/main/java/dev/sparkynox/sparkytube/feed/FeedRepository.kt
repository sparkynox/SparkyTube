 ...
 *
 * videoId is nullable now to allow Mix (Radio) and Playlist cards
 * through -- those often link to a /playlist?list=... URL with no bare
 * v= param for a "first track" id, so they have no plain videoId but
 * still need a working tap target (href + listId cover that case).
 * Previously any row without a parseable videoId was silently dropped
 * during scraping, which is why tapping a Mix/Playlist card in the feed
 * sometimes did nothing -- the row was visible but never actually made
 * it into the native list at all.
 */
            ...
            webView.evaluateJavascript(
                ...
                val parsed = try {
                    ...
                    (0 until itemsArray.length()).mapNotNull { i ->
                        ...
                        val videoId = item.optString("videoId").takeIf { it.length == 11 }
                        val listId = item.optString("listId").takeIf { it.isNotBlank() }
                        val href = item.optString("href").takeIf { it.isNotBlank() }
                        if (videoId == null && listId == null) return@mapNotNull null
                        FeedVideo(
                            videoId = videoId,
                            listId = listId,
                            href = href,
                ...
                if (continuation.isActive) continuation.resume(parsed, onCancellation = null)
            }
        }
