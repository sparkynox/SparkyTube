...
class MainActivity : AppCompatActivity(), JsBridge.VideoStateListener {
    ...
    private fun setupPlayerZoomGesture() {
        ...
        binding.exoPlayerView.setOnTouchListener { _, event ->
            ...
            // receive the event too — we're only adding zoom/pan on top,
            // not replacing normal tap-to-show-controls behavior.
            false
        }
    }
    ...
    /**
     * Shows a quality picker scoped to downloading — deliberately limited
     * to progressive streams (audioUrl == null in QualityOption), since
     * DownloadManager just saves whatever bytes are at one URL. Adaptive
     * (480p+) streams are video-only; downloading one of those directly
     * would produce a silent MP4. Progressive streams top out at 360p
     * (that's YouTube's own limit, not a SparkyTube restriction), so
     * that's the ceiling for downloads until a proper mux-on-device step
     * is added.
     */
    private fun showDownloadQualityPicker() {
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(labels) { _, which ->
                ...
                dev.sparkynox.sparkytube.download.VideoDownloader.downloadVideo(this, chosen.url, chosen.audioUrl, title)
            }
