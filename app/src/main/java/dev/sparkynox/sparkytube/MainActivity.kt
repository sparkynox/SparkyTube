...
class MainActivity : AppCompatActivity(), JsBridge.VideoStateListener {
    ...
    // Native feed screens (Home/Subs so far — Search also wires into
    // this). One FeedRepository per section since each tracks its own
    // "how many rows already consumed" scroll position independently;
    // switching sections swaps which repository+adapter pair is active
    // rather than sharing one across all of them.
    private var feedAdapter: dev.sparkynox.sparkytube.feed.FeedAdapter? = null
    private var homeFeedRepository: dev.sparkynox.sparkytube.feed.FeedRepository? = null
    private var subsFeedRepository: dev.sparkynox.sparkytube.feed.FeedRepository? = null
    private var shortsFeedRepository: dev.sparkynox.sparkytube.feed.FeedRepository? = null
    private var libraryFeedRepository: dev.sparkynox.sparkytube.feed.FeedRepository? = null
    // Search doesn't get a persistent slot like the others — every search
    // is a genuinely new query, so this is recreated fresh each time
    // rather than reused across searches the way Home/Subs's position
    // tracking makes sense to reuse.
    private var searchFeedRepository: dev.sparkynox.sparkytube.feed.FeedRepository? = null
    private var lastSearchUrl: String? = null
    private var activeFeedRepository: dev.sparkynox.sparkytube.feed.FeedRepository? = null
    private var isFeedLoadingMore = false
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Search") { _, _ ->
                ...
                if (query.isNotEmpty()) {
                    ...
                    // Search results use the same native feed list as
                    // Home/Subs/Shorts/Library — highlightNav doesn't
                    // apply here since none of the bottom nav tabs
                    // represent "search results". showNativeFeed already
                    // handles waiting for the WebView navigation to
                    // finish before its first scrape.
                    showNativeFeed(searchUrl)
                }
            }
            .setNegativeButton("Cancel", null)
            .show()
    ...
    private fun navigateToSection(navItem: LinearLayout, targetUrl: String) {
        ...
        val isNativeFeedSection = targetUrl == HOME_URL ||
            targetUrl.contains("/feed/subscriptions") ||
            targetUrl.contains("/shorts") ||
            targetUrl.contains("/feed/library")

        if (alreadyThere) {
            // Already on this URL -- if it's a native-feed section, still
            // make sure the native list (not the WebView) is what's
            // actually showing, since the user could have gotten here via
            // a different path (e.g. backed out of a video into Home).
            if (isNativeFeedSection) showNativeFeed(targetUrl)
            return
        }
        ...
        if (isNativeFeedSection) {
            showNativeFeed(targetUrl)
        } else {
            hideNativeFeed()
        }
    }
    ...
    /**
     * Switches the visible UI to the native feed RecyclerView for the
     * given section (Home or Subs) and starts loading it. The WebView
     * keeps running underneath, invisible — it's still the actual data
     * source FeedRepository scrapes from, just not what's on screen.
     */
    private fun showNativeFeed(sectionUrl: String) {
        if (feedAdapter == null) {
            feedAdapter = dev.sparkynox.sparkytube.feed.FeedAdapter(
                onItemClick = { videoId ->
                    browseMode = BrowseMode.YOUTUBE
                ...
                onNearEnd = { loadMoreFeedItems() }
            )
            binding.nativeFeedList.layoutManager = androidx.recyclerview.widget.LinearLayoutManager(this)
            binding.nativeFeedList.adapter = feedAdapter
        }

        val repository = when {
            sectionUrl == HOME_URL -> homeFeedRepository ?: dev.sparkynox.sparkytube.feed.FeedRepository(webView)
                .also { homeFeedRepository = it }
            sectionUrl.contains("/feed/subscriptions") -> subsFeedRepository ?: dev.sparkynox.sparkytube.feed.FeedRepository(webView)
                .also { subsFeedRepository = it }
            sectionUrl.contains("/shorts") -> shortsFeedRepository ?: dev.sparkynox.sparkytube.feed.FeedRepository(webView)
                .also { shortsFeedRepository = it }
            sectionUrl.contains("/feed/library") -> libraryFeedRepository ?: dev.sparkynox.sparkytube.feed.FeedRepository(webView)
                .also { libraryFeedRepository = it }
            sectionUrl.contains("/results") -> {
                // Every search is a fresh query — if this is a different
                // search than the last one, start a brand new repository
                // (and therefore a fresh scroll position) rather than
                // reusing the previous search's leftover state.
                if (lastSearchUrl != sectionUrl || searchFeedRepository == null) {
                    lastSearchUrl = sectionUrl
                    searchFeedRepository = dev.sparkynox.sparkytube.feed.FeedRepository(webView)
                }
        ...
        val isSwitchingSection = activeFeedRepository !== repository
        activeFeedRepository = repository
        binding.nativeFeedList.visibility = View.VISIBLE
        webView.visibility = View.INVISIBLE

        if (isSwitchingSection) {
            feedAdapter?.clear()
            repository.resetPosition()
            // Give the WebView a beat to actually finish navigating to
            // the new section before the first scrape — scraping
            // immediately after loadUrl() would just read the previous
            // page's leftover DOM.
            lifecycleScope.launch {
    ...
    private fun hideNativeFeed() {
        binding.nativeFeedList.visibility = View.GONE
        webView.visibility = View.VISIBLE
        activeFeedRepository = null
    }

    private fun loadMoreFeedItems() {
        val repository = activeFeedRepository ?: return
        if (isFeedLoadingMore) return
        isFeedLoadingMore = true
        lifecycleScope.launch {
            val newItems = repository.loadMore()
            feedAdapter?.appendItems(newItems)
