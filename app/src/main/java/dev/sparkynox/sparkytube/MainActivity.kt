...
class MainActivity : AppCompatActivity(), JsBridge.VideoStateListener {
    ...
    companion object {
        ...
        const val EXTRA_PROFILE_SWITCHED = "profile_switched"
    ...
    private fun applyMusicModeVisual() {
        ...
        contentFrame?.visibility = if (isMusicModeActive) View.INVISIBLE else View.VISIBLE
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Only redirect to the profile picker on a genuinely fresh launch
        // (no savedInstanceState -- e.g. not a rotation/config-change
        // recreate) with more than one profile, and never when we're the
        // ones being (re)created BY a profile switch itself
        // (EXTRA_PROFILE_SWITCHED) -- otherwise picking a profile would
        // bounce straight back to the picker instead of opening.
        val isProfileSwitchLaunch = intent.getBooleanExtra(EXTRA_PROFILE_SWITCHED, false)
        if (savedInstanceState == null && !isProfileSwitchLaunch &&
            dev.sparkynox.sparkytube.profile.ProfileManager.getProfiles(this).size > 1
        ) {
            startActivity(Intent(this, dev.sparkynox.sparkytube.profile.ProfilePickerActivity::class.java))
            finish()
            return
        }
        ...
        // If this Activity instance was (re)created by switching profiles
        // in ProfilePickerActivity, the new profile's cookie-jar needs to
        // load into the WebView's CookieManager before the very first
        // page load below -- clearing first so the outgoing profile's
        // session cookies can't linger and mix with the incoming one.
        if (isProfileSwitchLaunch) {
            val cookieManager = android.webkit.CookieManager.getInstance()
            dev.sparkynox.sparkytube.profile.ProfileManager.clearActiveCookies(cookieManager)
            dev.sparkynox.sparkytube.profile.ProfileManager.restoreCookiesForProfile(
                this,
                dev.sparkynox.sparkytube.profile.ProfileManager.getActiveProfileId(this),
                cookieManager
            )
        }
