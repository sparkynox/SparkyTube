...
class MainActivity : AppCompatActivity(), JsBridge.VideoStateListener {
    ...
    // once per video (see fetchSponsorSegmentsForCurrentVideo) rather
    // than re-fetched every poll tick.
    private var sponsorSegments: List<dev.sparkynox.sparkytube.sponsorblock.SponsorBlockManager.Segment> = emptyList()
    private var sponsorSegmentsFetchedForVideoId: String? = null
    private var lastSkippedSegmentEnd: Double = -1.0
        ...
        future.addListener({
            try {
                ...
                controller.addListener(object : Player.Listener {
                    override fun onIsPlayingChanged(isPlaying: Boolean) {
                        ...
                        if (isPlaying) {
                            ...
                            // Force-resume: playback stopped but the user
                            // never tapped pause -- most often a brief
                            // network hiccup or the stream stalling
                            // mid-buffer rather than a real end-of-video
                            // (STATE_ENDED is handled separately below and
                            // isn't affected by this). Keep retrying play()
                            ...
                            if (!userInitiatedPause && controller != null &&
                                ...
                                pollHandler.postDelayed({
                                    if (!userInitiatedPause && mediaController?.isPlaying == false &&
                ...
                pendingPlay?.let { pending ->
                    pendingPlay = null
                    playOnController(pending.streamUrl, pending.title, pending.isHls, pending.startPositionMs, pending.audioUrl, pending.videoId)
                }
            } catch (e: Exception) {
    ...
    // Force-resume: tracks whether the video is paused because the user
    // actually tapped pause, vs. paused for some other reason (buffering
    // stall recovering into a paused state, a transient network drop,
    // etc.). Only the former should stay paused -- anything else gets
    // resumed automatically. Defaults true so a freshly-loaded video
    // that hasn't started playing yet doesn't trigger an unwanted
    // auto-resume before playback has even begun.
    private var userInitiatedPause = true

    /**
     * Audio-only playback: hides just the video surface (exo_content_frame)
     * so the screen shows an album-art-style black area while audio keeps
     * playing through the same ExoPlayer/MediaSession -- no separate
     * playback pipeline, so background audio, the notification, and
     * lock-screen controls all keep working exactly as they do for video.
     * The invisible background WebView (real YouTube watch-history
     * tracking) is untouched by this -- it doesn't care whether the video
     * surface is visible on screen.
     */
    private var isMusicModeActive = false

    private fun toggleMusicMode() {
        isMusicModeActive = !isMusicModeActive
        // Global setting -- persists across videos and app restarts
        // until the user explicitly turns it off again, not a
        // per-video/per-session flag.
        dev.sparkynox.sparkytube.settings.SettingsPrefs.setMusicModeActive(this, isMusicModeActive)
        applyMusicModeVisual()
    }

    private fun applyMusicModeVisual() {
        val controller = mediaController

        if (isMusicModeActive) {
            // Actually disable the video track rather than just hiding
            // the surface -- setTrackTypeDisabled(TRACK_TYPE_VIDEO, true)
            // tells ExoPlayer to stop decoding/rendering video frames
            // entirely, not merely stop drawing them. This is what
            // removes the wasted CPU/battery/bandwidth cost of decoding
            // video the user can't even see; the old approach (just
            // hiding the surface) left the video decoder running the
            // whole time.
            controller?.trackSelectionParameters = controller?.trackSelectionParameters
                ?.buildUpon()
                ?.setTrackTypeDisabled(androidx.media3.common.C.TRACK_TYPE_VIDEO, true)
                ?.build() ?: androidx.media3.common.TrackSelectionParameters.DEFAULT

            // exoPlayerView itself stays VISIBLE (not hidden) -- only its
            // inner video surface (exo_content_frame) goes invisible, so
            // the thumbnail behind it shows through. Bug fix: this used
            // to hide the whole PlayerView and put the thumbnail as a
            // clickable overlay on top of it, which meant taps never
            // reached the actual play/pause/seek/music-mode controls
            // underneath -- there was no way to control playback at all
            // while in music mode. Keeping the real PlayerView on top and
            // touchable fixes that with no extra tap-forwarding needed.
            binding.exoPlayerView.visibility = View.VISIBLE
            val contentFrame = binding.exoPlayerView.findViewById<View>(
                androidx.media3.ui.R.id.exo_content_frame
            )
            contentFrame?.visibility = View.INVISIBLE
            binding.musicModeThumbnail.visibility = View.VISIBLE

            // Fallback background (moonlit night scene) instead of a flat
            // black placeholder/error state -- shown immediately while
            // the real thumbnail loads, and stays up if the thumbnail
            // request ever fails, so music mode never shows a plain
            // black screen.
            val thumbUrl = lastResolvedVideoId?.let { "https://i.ytimg.com/vi/$it/hqdefault.jpg" }
            if (thumbUrl != null) {
                binding.musicModeThumbnail.load(thumbUrl) {
                    crossfade(true)
                    placeholder(R.drawable.music_mode_bg)
                    error(R.drawable.music_mode_bg)
                }
            } else {
                binding.musicModeThumbnail.setImageResource(R.drawable.music_mode_bg)
            }
        } else {
            controller?.trackSelectionParameters = controller?.trackSelectionParameters
                ?.buildUpon()
            ...
            binding.exoPlayerView.visibility = if (isNativePlaybackActive) View.VISIBLE else View.GONE
            binding.musicModeThumbnail.visibility = View.GONE
        }

        val musicModeBtn = binding.exoPlayerView.findViewById<android.widget.ImageButton>(R.id.exo_music_mode_btn)
        musicModeBtn?.setColorFilter(
            if (isMusicModeActive) resources.getColor(R.color.accent, theme) else android.graphics.Color.WHITE
        )
    }

    // Cycles through these on tap rather than opening a picker dialog --
    // playback speed is changed often enough mid-video that a one-tap
    // cycle (matching how the button already shows the current value) is
    // faster than a dialog for the common case; the fixed step order
    // also makes the button's next value predictable without looking.
    private val PLAYBACK_SPEEDS = floatArrayOf(0.5f, 0.75f, 1f, 1.25f, 1.5f, 2f)
    private var currentPlaybackSpeed = 1f

    private fun formatSpeedLabel(speed: Float): String {
        // "1x" not "1.0x" for the common case -- 0.5x/1.5x still show
        // their decimal since dropping it there would be ambiguous.
        return if (speed == speed.toInt().toFloat()) "${speed.toInt()}x" else "${speed}x"
    }

    private fun cyclePlaybackSpeed(speedBtn: android.widget.TextView) {
        val currentIndex = PLAYBACK_SPEEDS.indexOfFirst { it == currentPlaybackSpeed }.coerceAtLeast(0)
        val nextIndex = (currentIndex + 1) % PLAYBACK_SPEEDS.size
        currentPlaybackSpeed = PLAYBACK_SPEEDS[nextIndex]
        speedBtn.text = formatSpeedLabel(currentPlaybackSpeed)
        mediaController?.setPlaybackSpeed(currentPlaybackSpeed)
    }
        ...
        val gestureDetector = android.view.GestureDetector(
            ...
            object : android.view.GestureDetector.SimpleOnGestureListener() {
                ...
                override fun onScroll(
                    ...
                    // Not zoomed -- vertical swipe controls volume (right
                    // half) or screen brightness (left half), same split as
                    // double-tap seek uses. Only treat it as a vol/brightness
                    // gesture once the swipe is clearly more vertical than
                    // horizontal, so an ordinary horizontal seek-scrub-ish
                    // drag doesn't accidentally change volume mid-swipe.
    ...
    // Total swipe distance (px) needed to go from 0% to 100% -- tuned so a
    // full top-to-bottom swipe across a typical phone screen roughly covers
    // the whole range, not so sensitive that a small scrub jumps volume/
    ...
    private fun adjustBrightness(distanceY: Float) {
        ...
        val currentBrightness = window.attributes.screenBrightness.let {
            if (it < 0f) 0.5f else it // -1 (system default) has no meaningful "current" value to nudge from
        }
        val delta = distanceY / gestureSwipeRangePx
        ...
        showGestureFeedback("Brightness", (newBrightness * 100).toInt())
    }

    /**
     * VidMate-style centered overlay: icon (speaker/brightness) + progress
     * bar + percent text + a mascot image, shown while actively swiping
     * and auto-hidden shortly after the gesture stops. Replaces the old
     * bottom Toast, which was easy to miss and didn't show a visual level.
     */
    private var gestureOverlayHideRunnable: Runnable? = null

    private fun showGestureFeedback(label: String, percent: Int) {
        val overlay = binding.gestureOverlay
        val icon = binding.gestureOverlayIcon
        val bar = binding.gestureOverlayBar
        val percentText = binding.gestureOverlayPercent
        val mascot = binding.gestureOverlayMascot

        val isVolume = label == "Volume"
        icon.setImageResource(
            if (isVolume) {
                if (percent <= 0) R.drawable.ic_volume_off else R.drawable.ic_volume_up
            } else {
                R.drawable.ic_brightness
            }
        )
        // Two cute mascot expressions -- the happier/excited one once the
        // level gets reasonably high, a shyer one otherwise. Purely a fun
        // touch, doesn't affect functionality either way.
        mascot.setImageResource(if (percent >= 50) R.drawable.mascot_happy else R.drawable.mascot_shy)

        bar.progress = percent.coerceIn(0, 100)
        percentText.text = "$label $percent%"

        overlay.visibility = View.VISIBLE
        overlay.alpha = 1f

        // Restart the auto-hide timer on every update so the overlay
        // stays visible for the whole swipe, only counting down once the
        // finger actually stops moving.
        gestureOverlayHideRunnable?.let { pollHandler.removeCallbacks(it) }
        val hideRunnable = Runnable {
            overlay.animate().alpha(0f).setDuration(200).withEndAction {
                overlay.visibility = View.GONE
            }.start()
        }
        gestureOverlayHideRunnable = hideRunnable
        pollHandler.postDelayed(hideRunnable, 700)
    }

    /**
     * Keeps videoTranslationX/Y within what the current zoom level
     * actually allows -- without this, panning (or zooming back out after
     * panning) could drag the video well past its own edges, leaving
     * black space or the video sitting outside the visible frame
     * entirely. Max pan offset in each direction is exactly how much
     * extra size the zoom added beyond the frame (half on each side,
     * since scaling happens from center).
     */
    private fun clampVideoTranslation() {
        val contentFrame = binding.exoPlayerView.findViewById<View>(
            androidx.media3.ui.R.id.exo_content_frame
        ) ?: binding.exoPlayerView
        val width = contentFrame.width.toFloat()
        val height = contentFrame.height.toFloat()
        if (width <= 0f || height <= 0f) return

        val maxX = (width * (videoScale - 1f)) / 2f
        val maxY = (height * (videoScale - 1f)) / 2f
        videoTranslationX = videoTranslationX.coerceIn(-maxX, maxX)
        videoTranslationY = videoTranslationY.coerceIn(-maxY, maxY)
    }

    private fun applyVideoTransform() {
        // Scale only the video surface (PlayerView's internal content
        // frame, id exo_content_frame) instead of the whole PlayerView --
        // scaling exoPlayerView itself was also scaling the controller
        // overlay (seek bar, buttons, title text) along with the video,
        // which is the "player itself zooms, not just the video" bug.
        val contentFrame = binding.exoPlayerView.findViewById<View>(
            androidx.media3.ui.R.id.exo_content_frame
        ) ?: binding.exoPlayerView
        contentFrame.scaleX = videoScale
        contentFrame.scaleY = videoScale
        contentFrame.translationX = videoTranslationX
        contentFrame.translationY = videoTranslationY
    }
    ...
    private fun setupCustomPlayerControls(attempt: Int = 0) {
        ...
        // Long-press opens the VidMate-style downloads/task manager
        // screen (progress, size, speed, per-download status) without
        // touching the existing tap-to-download behavior above.
        ...
        val musicModeBtn = binding.exoPlayerView.findViewById<View>(R.id.exo_music_mode_btn)
        musicModeBtn?.setOnClickListener { toggleMusicMode() }
        applyMusicModeVisual()

        val speedBtn = binding.exoPlayerView.findViewById<android.widget.TextView>(R.id.exo_speed_btn)
        speedBtn?.setOnClickListener { cyclePlaybackSpeed(speedBtn) }
        speedBtn?.text = formatSpeedLabel(currentPlaybackSpeed)
    ...
    /**
     * Download is effectively paused as of Aug 2026 — YouTube's SABR
     * enforcement has removed progressive (audio+video combined) streams
     * for most/all videos, and this affects every extractor (NewPipe,
     * yt-dlp, etc.), not just SparkyTube. NewPipeExtractor's own
     * "coordinating efforts on SABR implementation" issue has been open
     * since May 2025 with no full fix yet. Downloads only ever use a
     * progressive stream (see the note on VideoDownloader for why:
     * DownloadManager can't mux a separate video+audio pair), so when
     * NewPipe can't find one, there is currently nothing safe to
     * download — the picker isn't shown at all rather than offering
    ...
    private fun toggleFullscreenPlayer() {
        ...
        if (isPlayerFullscreen) {
            ...
            // ZOOM crops to fill the whole landscape frame edge-to-edge --
            // FIT (the portrait default) would letterbox with black bars
            // whenever the video's aspect ratio doesn't exactly match the
            // screen, which read as "zoomed out" compared to modern
            // players that fill fullscreen edge-to-edge by default.
            binding.exoPlayerView.resizeMode = androidx.media3.ui.AspectRatioFrameLayout.RESIZE_MODE_ZOOM
        } else {
            ...
            binding.exoPlayerView.resizeMode = androidx.media3.ui.AspectRatioFrameLayout.RESIZE_MODE_FIT

            // Bug fix: pinch-zoom/pan while in fullscreen was carrying
            // over into the normal (mini/portrait) player view on exit --
            // whatever scale/position the user left it at in fullscreen
            // stuck around and could show a cropped/off-center video, or
            // let the video be dragged outside the normal player's frame
            // entirely. Exiting fullscreen should always return to a
            // clean 1x centered state, same as the real YouTube app.
            videoScale = 1f
            videoTranslationX = 0f
            videoTranslationY = 0f
            applyVideoTransform()
    ...
    private suspend fun fetchRelatedVideosViaJs(): List<StreamExtractor.RelatedVideo> =
        kotlinx.coroutines.suspendCancellableCoroutine { continuation ->
            webView.evaluateJavascript(
                ...
                if (continuation.isActive) continuation.resume(parsed) { _, _, _ -> }
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Needed by the extraction fallback chain (Piped/yt-dlp toggles
        // live in SettingsPrefs, which needs a Context) -- must happen
        // before any resolvePlayableUrl() call, so it's first thing here.
        dev.sparkynox.sparkytube.extractor.StreamExtractor.attachContext(this)
        ...
        onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                ...
                if (isPlayerFullscreen) {
                    // Exit fullscreen back to the normal portrait layout
                    // instead of falling through to webView.goBack()/closing
                    // the app while still stuck in landscape -- that's the
                    // "back button leaves the app in landscape" bug.
    ...
    private fun downloadVideoById(videoId: String) {
        ...
        lifecycleScope.launch {
            ...
            dev.sparkynox.sparkytube.download.VideoDownloader.downloadVideo(
                ...
                resolved.title.ifBlank { videoId }, chosenOption.label, videoId
            )
        }
    }
    ...
    private fun showQualityPicker() {
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(labels, currentIndex) { dialog, which ->
                ...
                playOnController(streamUrl = chosen.url, title = currentTitle, isHls = false, startPositionMs = resumePosition, audioUrl = chosen.audioUrl, videoId = lastResolvedVideoId)
                dialog.dismiss()
            }
            .setNegativeButton("Cancel", null)
            .show()
    ...
    private data class PendingPlay(val streamUrl: String, val title: String, val isHls: Boolean, val startPositionMs: Long, val audioUrl: String?, val videoId: String? = null)
     ...
     */
    private fun playOnController(streamUrl: String, title: String, isHls: Boolean, startPositionMs: Long = 0L, audioUrl: String? = null, videoId: String? = null) {
        val controller = mediaController
        if (controller == null) {
            // Not connected yet — queue it. connectMediaController's listener
            // flushes this the moment the controller becomes available.
            pendingPlay = PendingPlay(streamUrl, title, isHls, startPositionMs, audioUrl, videoId)
            return
        }
        ...
        // Notification/lock-screen thumbnail — YouTube's standard thumbnail
        // CDN path works for any videoId with no extra API call needed.
        // Media3's DefaultMediaNotificationProvider loads this URI itself
        // (async, cached) purely from this being set on the MediaMetadata;
        // nothing else has to change for the artwork to show up.
        if (!videoId.isNullOrBlank()) {
            metadataBuilder.setArtworkUri(android.net.Uri.parse("https://i.ytimg.com/vi/$videoId/hqdefault.jpg"))
        }
    ...
    private fun setupWebView() {
        ...
        webView.settings.apply {
            ...
            // Data Saver toggles this at runtime too (see
            // applyDataSaverWebViewSettings) -- set here as the initial
            // state so a cold app start already respects the saved
            // preference before the first page even loads.
            loadsImagesAutomatically = !dev.sparkynox.sparkytube.settings.SettingsPrefs.isDataSaverEnabled(this@MainActivity)
        }
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun shouldInterceptRequest(
                ...
                if (dev.sparkynox.sparkytube.settings.SettingsPrefs.isDataSaverEnabled(this@MainActivity) &&
                    adBlockEngine.shouldBlockForDataSaver(url)
                ) {
                    return WebResourceResponse(
                        "text/plain",
                        "utf-8",
    ...
    private fun showNativeHomeFeed() {
        if (nativeFeedAdapter == null) {
            ...
            // Infinite scroll: fires loadMoreNativeFeed() a few rows
            // before the actual end so the next batch is usually ready
            // by the time the user scrolls that far, same pattern any
            // infinite-scroll feed uses. Previously the feed just stopped
            // dead after the first batch since nothing ever asked for
            // more -- this is the pagination fix.
            binding.nativeHomeFeedList.addOnScrollListener(object : androidx.recyclerview.widget.RecyclerView.OnScrollListener() {
            ...
            binding.nativeHomeFeedSwipeRefresh.setColorSchemeResources(R.color.accent)
            binding.nativeHomeFeedSwipeRefresh.setOnRefreshListener {
                fetchAndPopulateNativeFeed(showSpinner = false)
            }
        }

        binding.nativeHomeFeedSwipeRefresh.visibility = View.VISIBLE
        webView.visibility = View.INVISIBLE
        nativeFeedContinuationToken = null
        isLoadingMoreNativeFeed = false

        fetchAndPopulateNativeFeed(showSpinner = true)
    }

    /**
     * Shared fetch-and-populate for both the initial feed load (spinner)
     * and pull-to-refresh (SwipeRefreshLayout's own spinner instead).
     */
    private fun fetchAndPopulateNativeFeed(showSpinner: Boolean) {
        if (showSpinner) binding.loadingSpinner.visibility = View.VISIBLE

        lifecycleScope.launch {
            val result = dev.sparkynox.sparkytube.homefeed.InnerTubeClient.fetchHomeFeed()
            binding.loadingSpinner.visibility = View.GONE
            binding.nativeHomeFeedSwipeRefresh.isRefreshing = false
            ...
            nativeFeedContinuationToken = result.continuationToken
            nativeFeedAdapter?.submitItems(result.items)
        }
    }

    private var nativeFeedContinuationToken: String? = null
    private var isLoadingMoreNativeFeed = false

    /**
     * Fetches and appends the next batch using the continuation token
     * from the last fetch -- this is what makes the feed's scroll
     * genuinely infinite instead of stopping after one screenful.
     * isLoadingMoreNativeFeed guards against the scroll listener firing
     * this multiple times before the first request even finishes (easy
     * to trigger with a fast fling).
     */
    private fun loadMoreNativeFeed() {
        val token = nativeFeedContinuationToken ?: return
        if (isLoadingMoreNativeFeed) return
        isLoadingMoreNativeFeed = true
        ...
        lifecycleScope.launch {
            ...
            if (result.items.isNotEmpty()) {
                nativeFeedAdapter?.appendItems(result.items)
            }
        }
    }
    ...
    /**
     * Fast path: asks injected.js for whatever streamingData YouTube's own
     * page already parsed into memory (see __sparkyGetFastStreamData) --
     * this is a same-thread JS lookup, not a network round-trip, so it
     * resolves in milliseconds when it works. Only covers plain signed
     * URLs (see the JS-side comment for why cipher-scrambled streams are
     * skipped); returns null on any miss, cipher case, or parse failure,
     * which the caller falls through to the normal NewPipeExtractor path
     * for -- so this only ever makes things faster, never less reliable.
     */
    /**
     * Master resolve function — decides the order to try extraction
     * methods in based on Settings > Extractor Method (see
     * SettingsPrefs.ExtractorMethod), then falls through each candidate
     * until one succeeds:
     *
     *  - AUTO: local server (only if enabled in Local Servers settings)
     *          -> JS fast path -> NewPipeExtractor
     *  - LOCAL_SERVER: local server only -- no fallback, since this
     *          exists for people who deliberately want to know if their
     *          server setup is broken rather than silently falling back
     *  - NEWPIPE: skips both the local server and the JS fast path
     *          entirely, straight to NewPipeExtractor
     *
     * The local server is only ever attempted if SettingsPrefs.isLocalServerEnabled
     * is true, regardless of ExtractorMethod -- LOCAL_SERVER mode with the
     * toggle off is treated the same as NEWPIPE mode's "nothing to try",
     * i.e. extraction just fails for that case rather than silently
     * reaching out to a server the person hasn't turned on.
     */
    private suspend fun resolveStream(videoId: String): StreamExtractor.ResolvedStream? {
        val method = dev.sparkynox.sparkytube.settings.SettingsPrefs.getExtractorMethod(this)
        val localServerEnabled = dev.sparkynox.sparkytube.settings.SettingsPrefs.isLocalServerEnabled(this)

        suspend fun tryLocalServer(): StreamExtractor.ResolvedStream? {
            if (!localServerEnabled) return null
            val baseUrl = dev.sparkynox.sparkytube.settings.SettingsPrefs.getLocalServerUrl(this)
            val apiKey = dev.sparkynox.sparkytube.settings.SettingsPrefs.getLocalServerApiKey(this)
            return StreamExtractor.resolveViaLocalServer(videoId, baseUrl, apiKey)
        }

        return when (method) {
            dev.sparkynox.sparkytube.settings.SettingsPrefs.ExtractorMethod.LOCAL_SERVER ->
                tryLocalServer()
    ...
    private suspend fun tryFastResolve(videoId: String): StreamExtractor.ResolvedStream? =
        kotlinx.coroutines.suspendCancellableCoroutine { continuation ->
            webView.evaluateJavascript(
                "(function(){return window.__sparkyGetFastStreamData ? " +
                    "window.__sparkyGetFastStreamData(${org.json.JSONObject.quote(videoId)}) : null;})();"
            ) { rawResult ->
                val resolved = try {
                    val unwrapped = org.json.JSONTokener(rawResult ?: "null").nextValue() as? String
                    unwrapped?.let { StreamExtractor.parseFastStreamData(it) }
                        ?.also { StreamExtractor.cacheResolved(videoId, it) }
                } catch (e: Exception) {
                    null
                }
                if (continuation.isActive) continuation.resume(resolved) { _, _, _ -> }
            }
        }
    ...
    private fun resolveAndPlayNative(videoId: String, fallbackTitle: String) {
        ...
        isMusicModeActive = dev.sparkynox.sparkytube.settings.SettingsPrefs.isMusicModeActive(this)
        currentPlaybackSpeed = 1f
        userInitiatedPause = false
        binding.exoPlayerView.findViewById<android.widget.TextView>(R.id.exo_speed_btn)?.text = formatSpeedLabel(1f)
        lifecycleScope.launch {
            ...
            // picked. This decides the actual playback URL below (not just
            // the displayed label) -- the person can still bump the
            // quality up from the picker for this one video same as usual.
            val dataSaverQuality = if (isDataSaverOn) {
                resolved.availableQualities.minByOrNull { it.resolutionValue }
            } else null

            currentQualityLabel = dataSaverQuality?.label
                ?: resolved.defaultQualityLabel ?: resolved.availableQualities.firstOrNull()?.label ?: ""
    ...
    private fun showPlaybackErrorDialog() {
        ...
        // If yt-dlp is the explicitly-selected Extractor Method (not just
        // the last link in AUTO's fallback chain), show its actual error
        // instead of the generic message -- a silent "couldn't play"
        // with no detail was the actual bug report here: yt-dlp WAS
        ...
        if (method == dev.sparkynox.sparkytube.settings.SettingsPrefs.ExtractorMethod.YT_DLP) {
            ...
            AlertDialog.Builder(this)
                ...
                .setPositiveButton("OK", null)
                .setNeutralButton("Check Logs") { _, _ ->
     ...
     */
    /**
     * Fetches SponsorBlock segments once per video (called from
     * resolveAndPlayNative right after lastResolvedVideoId is set) rather
     * than on every poll tick -- a single network call per video is
     * plenty, segments don't change mid-playback.
     */
    private fun fetchSponsorSegmentsForCurrentVideo(videoId: String) {
        sponsorSegments = emptyList()
        lastSkippedSegmentEnd = -1.0
        sponsorSegmentsFetchedForVideoId = videoId

        if (!dev.sparkynox.sparkytube.settings.SettingsPrefs.isSponsorBlockEnabled(this)) return

        val categories = dev.sparkynox.sparkytube.settings.SettingsPrefs.getSponsorBlockCategories(this)
        lifecycleScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            val segments = dev.sparkynox.sparkytube.sponsorblock.SponsorBlockManager.fetchSegments(videoId, categories)
            kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
                // Guard against a slower fetch landing after the user's
                // already moved to a different video.
                if (sponsorSegmentsFetchedForVideoId == videoId) {
    ...
    /**
     * Checks the player's current position against fetched segments and
     * seeks past any segment the playhead is currently inside. Only
     * skips forward (never re-skips a segment the user manually
     * rewound back into on purpose -- lastSkippedSegmentEnd tracks the
     * most recent skip so scrubbing backward into an already-skipped
     * segment doesn't immediately yank forward again).
     */
    private fun maybeSkipSponsorSegment() {
        if (sponsorSegments.isEmpty()) return
        val controller = mediaController ?: return
        if (!controller.isPlaying) return

        val positionSeconds = controller.currentPosition / 1000.0
        val segment = sponsorSegments.firstOrNull { positionSeconds >= it.startSeconds && positionSeconds < it.endSeconds } ?: return

        if (segment.endSeconds <= lastSkippedSegmentEnd) return // already skipped this one

        controller.seekTo((segment.endSeconds * 1000).toLong())
        lastSkippedSegmentEnd = segment.endSeconds
    ...
    override fun onUserLeaveHint() {
        ...
        // onUserLeaveHint fires specifically for Home-button/recents-style
        // "leaving the app" gestures -- unlike onPause, which also fires
        // for things like a dialog or another Activity briefly taking
        // focus, where auto-entering PIP would be wrong. Only offer PIP
        // when a video is actually playing full-size right now; there's
        // nothing to show in a PIP window during Music Mode (no visible
        // video surface) or the mini-player (already a small window) or
        // when nothing is playing at all.
        if (isNativePlaybackActive && !isMiniPlayerActive && !isMusicModeActive) {
            enterPipMode()
        }
    }

    /**
     * Picture-in-Picture entry point -- called both from onUserLeaveHint
     * (automatic, Home button) and from a manual trigger if one is added
     * to the player controls later. Aspect ratio comes from the actual
     * video dimensions ExoPlayer reports so the PIP window isn't
     * letterboxed/pillarboxed for non-16:9 content (Shorts-shaped
     * uploads, etc.) -- falls back to 16:9 if the player hasn't reported
     * a size yet (e.g. still buffering the very first frame).
     */
    private fun enterPipMode() {
        ...
        val aspectRatio = if (videoSize != null && videoSize.width > 0 && videoSize.height > 0) {
            android.util.Rational(videoSize.width, videoSize.height)
        } else {
        ...
        try {
            ...
            // Some OEM launchers/devices reject PIP entry in edge cases
            // (e.g. a device policy disabling it) -- fail silently rather
            // than crashing on what's meant to be a convenience feature.
        }
    }
    ...
    override fun onPictureInPictureModeChanged(
        ...
        // The player controls overlay (seek bar, buttons) has no purpose
    ...
    override fun onResume() {
        ...
        // Picks up a Data Saver toggle flipped on the Settings screen
        // while this Activity was paused (Settings is a separate
        // Activity, so it can't reach into this WebView instance
        // directly) -- same "refresh from prefs on resume" pattern
