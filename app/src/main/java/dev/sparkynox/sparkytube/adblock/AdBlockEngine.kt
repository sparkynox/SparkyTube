...
class AdBlockEngine private constructor(
    ...
    // streams video/audio bytes from. This is what makes "the default
    // YouTube player never loads" actually true rather than just visually
    // hidden: the WebView's <video> element can request a stream all it
    // wants, but the request itself gets blocked at the network layer, so
    // no media data ever arrives. Always on — SparkyTube never falls back
    // to WebView video playback; if native extraction fails, MainActivity
    // shows an error instead (see resolveAndPlayNative).
    //
    // NewPipeExtractor itself is unaffected: it fetches streams through its
    // own native HTTP client (SparkyDownloader in StreamExtractor.kt), not
    // through the WebView, so this block never blocks native playback.
