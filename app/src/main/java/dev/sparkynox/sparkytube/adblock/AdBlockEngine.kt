...
class AdBlockEngine private constructor(
    ...
    private val dataSaverPaths: List<String>
) {
    ...
    /**
     * Separate from shouldBlock() above -- these aren't ads, they're
     * YouTube's own telemetry/prefetch/hover-preview requests (watch-time
     * pings, hover-thumbnail prefetch, feedback beacons). Blocking them
     * doesn't touch which video plays or the real watch-history signal
     * MainActivity's dual-player setup deliberately preserves (see the
     * big comment above shouldBlock, "the fix that IS legitimate") --
     * only the extra chatter around it. Only called when Settings > Data
     * Saver Mode is on, never mixed into the always-on ad-block path.
     */
    fun shouldBlockForDataSaver(url: String): Boolean {
        val lower = url.lowercase()
        for (p in dataSaverPaths) {
            if (lower.contains(p)) {
                blockedCount.incrementAndGet()
                return true
            }
        ...
        private fun buildFromAssets(context: Context): AdBlockEngine {
            return try {
                ...
                AdBlockEngine(
                    ...
                    dataSaverPaths = obj.optJSONArray("dataSaverPaths")?.toStringList() ?: emptyList()
                )
            } catch (e: Exception) {
                ...
                AdBlockEngine(
                    ...
                    dataSaverPaths = listOf("/api/stats/watchtime", "/generate_204", "/log_event")
                )
            }
        }
