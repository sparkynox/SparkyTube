...
(function () {
  ...
  window.__sparkyGetRelatedVideosJson = function () {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      results.push({
        ...
        uploaderName: extractRowChannel(row),
        durationText: extractRowDuration(row),
        thumbnailUrl: extractRowThumbnail(row)
      });
    }
    return JSON.stringify(results);
  };

  // Tries each selector in order, returns the first element found (or
  // null if none match) -- used throughout the feed scraper so a single
  // YouTube markup change doesn't blank out a whole field, just falls
  // through to the next known pattern.
  ...
  function extractRowTitle(row, link) {
    ...
    // aria-label/title attributes are accessibility metadata YouTube
    // renders consistently regardless of which CSS class names are in
    // fashion this month -- a much more stable fallback than any
    // particular class selector.
    if (link) {
      var aria = link.getAttribute('aria-label') || link.getAttribute('title');
      if (aria) return aria.trim();
    }
    return '';
  }

  function extractRowChannel(row) {
    var channelEl = queryFirst(row, [
      '.ytm-channel-name',
  ...
  function extractRowDuration(row) {
    var durationEl = queryFirst(row, [
  ...
  function extractRowViews(row) {
    var viewsEl = queryFirst(row, [
      '.yt-content-metadata-view-model-wiz__metadata-text',
      '.ytm-video-metadata-renderer',
      '[class*="metadata-text"]'
    ]);
    return viewsEl ? viewsEl.textContent.trim() : '';
  }

  function extractRowThumbnail(row) {
    var thumbEl = row.querySelector('img');
    if (!thumbEl) return '';
    // Lazy-loaded thumbnails often sit in data-src until they scroll into
    // view; src can be a placeholder/blank data URI until then, so
    // data-src is checked FIRST here (opposite priority from the related-
    // videos panel, which scrapes rows already fully on-screen and
    // therefore already real-src'd).
    return thumbEl.getAttribute('data-src') || thumbEl.src || '';
  }

  // Generic feed scraper for Home / Search / Subscriptions -- the native
  // Home/Search/Subs screens use this instead of the WebView's own
  // rendering, reading whatever YouTube already loaded into the DOM.
  // Covers both card-grid rows (ytm-rich-item-renderer, what the Home
  // feed uses) and compact list rows (ytm-video-with-context-renderer,
  // what Search results and Subscriptions use) with one function, since
  // the underlying video info (id/title/channel/duration/thumbnail) is
  // the same shape either way.
  //
  // sinceIndex lets the native side ask for "everything after what I
  // already have" rather than re-parsing and re-sending the whole DOM on
  // every scroll tick -- native tracks how many items it already
  // received and passes that count back in.
  window.__sparkyGetFeedVideosJson = function (sinceIndex) {
    var rows = document.querySelectorAll(
      'ytm-rich-item-renderer, ytm-video-with-context-renderer, ytm-compact-video-renderer, ' +
      'ytm-shorts-lockup-view-model, ytm-radio-renderer, ytm-compact-playlist-renderer, ytm-playlist-renderer'
    );
    var results = [];
    var startAt = typeof sinceIndex === 'number' ? sinceIndex : 0;

    for (var i = startAt; i < rows.length; i++) {
      var row = rows[i];
      var link = row.querySelector('a');
      var href = link ? link.href : null;
      var videoId = extractVideoIdFromHref(href);

      // Mix (Radio) and Playlist cards often link straight to
      // /playlist?list=... or a watch link with only a `list=` param and
      // no bare `v=` for the first track -- extractVideoIdFromHref
      // returns null for those, and the row used to just get silently
      // dropped here (`continue`), which is why tapping a Mix/Playlist
      // card in the feed sometimes did nothing: the card was visible
      // (still WebView-rendered leftover DOM) but never actually made it
      // into the native feed list at all, so there was no native click
      // target wired up for it. Now these fall back to using the raw
      // href + a list= id instead of being skipped.
      var listMatch = href ? href.match(/[?&]list=([a-zA-Z0-9_-]+)/) : null;
      var listId = listMatch ? listMatch[1] : null;
      if (!videoId && !listId) continue;
      ...
      results.push({
        ...
        durationText: extractRowDuration(row),
        thumbnailUrl: extractRowThumbnail(row),
        viewsText: extractRowViews(row)
      });
    }
    return JSON.stringify({ items: results, totalRowsSeen: rows.length });
  };

  // Scrolls the (invisible, background) WebView itself so YouTube's own
  // infinite-scroll loads more items -- the whole point of running
  // m.youtube.com in the background at all: native scrolling drives this
  // to keep the feed's underlying data fresh, even though the user never
  // sees this WebView directly. Scrolls by a fraction of the page height
  // rather than a fixed pixel count so it behaves consistently across
  // different screen sizes.
  window.__sparkyScrollFeed = function () {
  ...
  /**
   * Fast-path stream extraction: YouTube's own page already fetched and
   * parsed ytInitialPlayerResponse (or ytplayer.config on some mobile page
   * variants) into memory before this script even runs. Reading it out
   * directly is a same-thread object lookup -- no network round-trip, no
   * HTML parse -- versus StreamExtractor.kt's NewPipe path, which re-fetches
   * and re-parses the whole watch page from scratch on the native side.
   *
   * Only handles the common case: a signed, ready-to-use progressive or
   * adaptive URL already present in streamingData. Some videos serve
   * cipher-scrambled URLs (needing YouTube's player.js signature/n-param
   * transform to decode) instead of a plain "url" field -- those are
   * deliberately left alone here (no attempt to reimplement YouTube's
   * cipher) and just fall through to null, which tells the Kotlin side to
   * fall back to NewPipeExtractor as usual. So this speeds up the common
   * case without touching the fallback path's correctness.
   */
  function __sparkyGetFastStreamData(videoId) {
    try {
      var data = window.ytInitialPlayerResponse ||
        (window.ytplayer && window.ytplayer.config && window.ytplayer.config.args &&
          JSON.parse(window.ytplayer.config.args.player_response || 'null'));
      if (!data || !data.streamingData) return null;

      // Make sure the player data actually matches the video we're being
      // asked about -- ytInitialPlayerResponse can lag behind window.location
      // by a beat during a fast SPA nav between two /watch pages.
      var responseVideoId = data.videoDetails && data.videoDetails.videoId;
      ...
      // Only plain signed URLs count as "fast" -- cipher-scrambled streams
      // carry `signatureCipher`/`cipher` instead of `url` and need
      // YouTube's obfuscated player.js transform to decode, which this
      // fast path intentionally does not attempt.
      var usable = formats.filter(function (f) { return !!f.url; });
      if (usable.length === 0) return null;

      var progressive = usable.filter(function (f) {
        return f.mimeType && f.mimeType.indexOf('video/') === 0 && f.audioQuality;
      });
      var adaptiveVideo = usable.filter(function (f) {
        return f.mimeType && f.mimeType.indexOf('video/') === 0 && !f.audioQuality;
      });
      var adaptiveAudio = usable.filter(function (f) {
        return f.mimeType && f.mimeType.indexOf('audio/') === 0;
      });

      function toQuality(f, audioUrl) {
        return {
          label: (f.qualityLabel || (f.height ? f.height + 'p' : 'audio')),
          url: f.url,
          resolutionValue: f.height || 0,
          audioUrl: audioUrl || null
        };
      }

      var bestAudio = adaptiveAudio.sort(function (a, b) { return (b.bitrate || 0) - (a.bitrate || 0); })[0];
      var qualities = progressive.map(function (f) { return toQuality(f, null); })
        .concat(adaptiveVideo.map(function (f) { return toQuality(f, bestAudio ? bestAudio.url : null); }));

      if (qualities.length === 0) return null;

      return JSON.stringify({
        title: (data.videoDetails && data.videoDetails.title) || document.title.replace(' - YouTube', ''),
        durationSeconds: (data.videoDetails && parseInt(data.videoDetails.lengthSeconds, 10)) || 0,
        qualities: qualities
      });
    } catch (e) {
      return null;
  ...
  window.__sparkyGetFastStreamData = __sparkyGetFastStreamData;
